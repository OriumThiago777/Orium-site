# /relatorio Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Construir a ferramenta interna /relatorio — formulário em 7 etapas para relatórios mensais de clientes, com preview premium e exportação PDF/texto.

**Architecture:** Single client component `app/relatorio/page.tsx` com três estados: tela de senha → formulário 7 etapas (sidebar + conteúdo) → preview + exportação. Nenhuma rota de API nova. PDF gerado com jsPDF + html2canvas (já instalados).

**Tech Stack:** Next.js 15 App Router, TypeScript, Tailwind (classes mínimas), inline styles seguindo padrão briefing, jsPDF, html2canvas, lib/auth.ts
...
|---------|------|-----------------|
| `app/relatorio/page.tsx` | Criar | Página completa: auth, formulário, preview, exportação |
| `app/hub/page.tsx` | Modificar | Mover card RELATÓRIO de EM_BREVE para FERRAMENTAS |

---

### Task 1: Ativar card RELATÓRIO no Hub

**Files:**
- Modify: `app/hub/page.tsx`

- [ ] **Step 1: Abrir `app/hub/page.tsx` e localizar os arrays**

Encontre as duas constantes no topo do arquivo:
```ts
const FERRAMENTAS = [ ... ]   // cards ativos com href
const EM_BREVE = [ ... ]      // cards desativados
```

- [ ] **Step 2: Remover RELATÓRIO de EM_BREVE**

Apague o objeto `{ tag: 'EM BREVE', titulo: 'RELATÓRIO', ... }` do array `EM_BREVE`.

- [ ] **Step 3: Adicionar RELATÓRIO em FERRAMENTAS**

Adicione ao final do array `FERRAMENTAS`:
```ts
{
  tag: 'RESULTADOS',
  titulo: 'RELATÓRIO',
...
Inicie o servidor com `npm run dev` e acesse `http://localhost:3000/hub` (após autenticar). O card RELATÓRIO deve aparecer com hover laranja e ser clicável, não mais opaco.

- [ ] **Step 5: Commit**

```bash
git add app/hub/page.tsx
git commit -m "feat: ativar card /relatorio no hub"
```

---

### Task 2: Estrutura base e tela de senha

**Files:**
- Create: `app/relatorio/page.tsx`

- [ ] **Step 1: Criar o arquivo com imports e state inicial**

Crie `app/relatorio/page.tsx` com o conteúdo abaixo. Este é o esqueleto completo do state — não altere as assinaturas de state em tasks futuras:
...
Acesse `http://localhost:3000/relatorio`. Deve aparecer a tela de senha com visual premium. Após autenticar, aparece "Em construção…" em branco — isso é esperado neste ponto.

- [ ] **Step 3: Commit**

```bash
git add app/relatorio/page.tsx
git commit -m "feat: estrutura base e tela de senha do /relatorio"
```

---

### Task 3: Sidebar e layout principal

**Files:**
- Modify: `app/relatorio/page.tsx`

- [ ] **Step 1: Substituir o return placeholder pelo layout de duas colunas**

Substitua a linha `return <div style={{ color: '#fff', padding: '2rem', fontFamily: 'Poppins' }}>Em construção…</div>` pelo bloco abaixo. Este layout persiste em todas as etapas (incluindo o preview):
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', background: '#080808', fontFamily: 'Poppins, sans-serif', display: 'flex' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', position: 'relative', zIndex: 1 }}>
        ...
        {/* corpo — preenchido nas tasks seguintes */}
        <div ref={contentRef} style={{ flex: 1, overflowY: 'auto', padding: isPreview ? '0' : '3rem 5rem' }}>
          <p style={{ color: '#555' }}>Campos virão aqui…</p>
        </div>
...
Acesse `http://localhost:3000/relatorio`. A sidebar deve aparecer com 7 etapas numeradas, toggle funcional, barra de progresso, link ← PAINEL. Clicar "CONTINUAR →" deve avançar as etapas (mesmo sem campos por enquanto). A etapa 7 deve mostrar "VER RELATÓRIO".

- [ ] **Step 3: Commit**

```bash
git add app/relatorio/page.tsx
git commit -m "feat: sidebar e layout principal do /relatorio"
```

---

### Task 4: Campos das etapas 1–3

**Files:**
- Modify: `app/relatorio/page.tsx`

- [ ] **Step 1: Criar a função `renderStep` e substituir o placeholder do corpo**

Substitua `<p style={{ color: '#555' }}>Campos virão aqui…</p>` por:

```tsx
          {!isPreview && renderStep()}
```

Adicione a função `renderStep` antes do `return` principal (após as funções utilitárias existentes):
...
```tsx
  ...
  function renderStep() {
    ...
    return null // steps 3–6 implementados na task seguinte
  }
```

- [ ] **Step 2: Verificar no browser**

Navegue pelas etapas 1, 2, 3. Etapa 1 deve ter três campos (cliente, período, responsável). Etapa 2 deve ter input + botão + para adicionar entregas como tags. Etapa 3 deve ter seguidores lado a lado + 4 inputs numéricos. Etapas 4–7 aparecem em branco (esperado).

- [ ] **Step 3: Commit**

```bash
git add app/relatorio/page.tsx
git commit -m "feat: campos das etapas 1-3 do /relatorio"
```

---

### Task 5: Campos das etapas 4–7

**Files:**
- Modify: `app/relatorio/page.tsx`

- [ ] **Step 1: Substituir `return null` no final de `renderStep` pelos steps 3–6**

Substitua a linha `return null // steps 3–6 implementados na task seguinte` por:
...
Navegue por todas as 7 etapas. Na etapa 7, ao clicar "VER RELATÓRIO", o step vai para 7 e a área de conteúdo fica vazia (ainda sem preview). Isso é esperado.

- [ ] **Step 3: Commit**

```bash
git add app/relatorio/page.tsx
git commit -m "feat: campos das etapas 4-7 do /relatorio"
```

---

### Task 6: Preview do relatório

**Files:**
- Modify: `app/relatorio/page.tsx`

- [ ] **Step 1: Substituir o conteúdo quando `isPreview` é true**

No corpo da área de conteúdo, o bloco atual é:
```tsx
        <div ref={contentRef} style={{ flex: 1, overflowY: 'auto', padding: isPreview ? '0' : '3rem 5rem' }}>
          {!isPreview && renderStep()}
        </div>
...
- [ ] **Step 2: Adicionar a função `renderPreview` antes do `return` principal**

Adicione após a função `renderStep`:
        ...
        <div ref={previewRef} style={{ background: '#0d0d0d', borderRadius: '12px', padding: '2.5rem', border: '1px solid #1a1a1a' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
            {[
              ...
              { label: 'ALCANCE', value: form.alcance || '—' },
              { label: 'IMPRESSÕES', value: form.impressoes || '—' },
              { label: 'ENGAJAMENTO', value: form.engajamento ? `${form.engajamento}%` : '—' },
              { label: 'CLIQUES', value: form.cliques || '—' },
            ].map(card => (
...
- [ ] **Step 3: Adicionar footer de exportação quando `isPreview`**

Após o bloco `{!isPreview && (...footer...)}` no JSX principal, adicione:
...
Preencha dados nas etapas e clique "VER RELATÓRIO". O preview deve aparecer com header, cards de métricas em laranja, seções com separadores, botões COPIAR e EXPORTAR PDF no footer. Os botões ainda não funcionam (implementados na task seguinte).

- [ ] **Step 5: Commit**

```bash
git add app/relatorio/page.tsx
git commit -m "feat: preview do relatório mensal"
```

---

### Task 7: Copiar e exportar PDF

**Files:**
- Modify: `app/relatorio/page.tsx`

- [ ] **Step 1: Adicionar `handleCopiar` antes do `return` principal**
...
- [ ] **Step 2: Adicionar `handleExportarPDF` antes do `return` principal**
    ...
    try {
      ...
      if (contentH <= pageH - margin * 2) {
        ...
        while (y < totalH) {
          ...
          doc.addImage(imgData, 'PNG', margin, margin - (y * (pageW / contentW)), contentW, contentH)
          y += sliceH
        }
      }
...
- [ ] **Step 3: Verificar botão Copiar**

Na tela de preview, clique "COPIAR RELATÓRIO". O botão deve mudar para "✓ COPIADO" por 2 segundos. Cole em qualquer editor de texto — o conteúdo formatado deve aparecer com todas as seções.

- [ ] **Step 4: Verificar exportação PDF**

Clique "EXPORTAR PDF". Deve aparecer "GERANDO..." brevemente e então baixar um arquivo `relatorio-[cliente]-[mes]-[ano].pdf`. Abra o PDF e confirme que o conteúdo do preview está renderizado.

- [ ] **Step 5: Commit final**

```bash
git add app/relatorio/page.tsx
git commit -m "feat: copiar texto e exportar PDF no /relatorio"
```
...
- [ ] Acessar `/hub` → card RELATÓRIO clicável, hover laranja, tag "RESULTADOS"
- [ ] Acessar `/relatorio` sem sessão → tela de senha aparece
- [ ] Autenticar → formulário abre na etapa 1
- [ ] Navegar pelas 7 etapas com CONTINUAR/VOLTAR
- [ ] Sidebar colapsa/expande, link ← PAINEL funciona
- [ ] Etapa 2 e 6: adicionar e remover itens de lista
- [ ] Etapa 3: campos numéricos lado a lado
- [ ] VER RELATÓRIO → preview completo com métricas, listas, seções
- [ ] COPIAR RELATÓRIO → clipboard com texto formatado
- [ ] EXPORTAR PDF → download do arquivo correto
- [ ] "← Editar dados" → volta à etapa 1
- [ ] Abrir `/relatorio` com sessão ativa → vai direto ao formulário (sem senha)
